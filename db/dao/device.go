...
func (dd deviceDao) EnsureDevice(
	...
	if ok {
		// Update device
		_, err := global.App.DB.Device.UpdateOne(oldDevice).
		...
		// Create device
		d, err := global.App.DB.Device.Create().
			SetIdentifier(identifier).
			SetChannel(channel).
			SetChannelToken(channelToken).
			SetDeviceName(deviceName).
...
func (deviceDao) UpdateDeviceChannelMeta(ctx context.Context, d *generate.Device, channelMeta string) bool {
	_, err := global.App.DB.Device.UpdateOne(d).SetChannelMeta(channelMeta).Save(ctx)
	if err != nil {
...
func (deviceDao) DeleteDeviceByIdentifier(ctx context.Context, identifier string) bool {
	_, err := global.App.DB.Device.Delete().Where(device.Identifier(identifier)).Exec(ctx)
	if err != nil {
		if !generate.IsNotFound(err) {
...
func (deviceDao) DeleteDevice(ctx context.Context, d *generate.Device) bool {
	err := global.App.DB.Device.DeleteOne(d).Exec(ctx)
	if err != nil {
		if !generate.IsNotFound(err) {
